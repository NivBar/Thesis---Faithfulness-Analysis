...
if __name__ == '__main__':
    ...
    if not online:
        # greg_df = pd.read_csv(f'/lv_local/home/niv.b/llama/tommy_data.csv').astype(str) #TOMMY
    ...
    for idx, row in df.iterrows():
        ...
        #     if names_nto[re.sub(r"-(\d)-", lambda m: f"-0{m.group(1)}-", row.previous_docno_str)] in data_dict.keys():
    ...
    for idx, row in tqdm(df.iterrows(), total=df.shape[0]):
        ...
        # if row.previous_docno_str not in data_dict.keys():  # EF_DEBUG
        #     if names_nto[row.previous_docno_str.replace("-2-","-02-")] in data_dict.keys():
        #         df.at[idx, 'previous_docno_str'] = names_nto[row.previous_docno_str.replace("-2-","-02-")]
        #     else:
        ...
        try:
            ...
            try:
                ...
                try:
                    text = greg_df[greg_df.new_docno == row.docno.replace("-" + row.docno.split("-")[1] + "-",
                                                                              "-" + row.docno.split("-")[1].zfill(
                                                                                  3) + "-").replace("00","0")].current_document.values[0]
                except:
                    text = greg_df[greg_df.new_docno ==  re.sub(r"-(\d)-", lambda m: f"-0{m.group(1)}-", row.docno)].current_document.values[0]
                ...
                if 'previous_docno_str' in df.columns and row.previous_docno_str != 'nan':
                    try:
                        if online:
                            ...
                            # prev_text = greg_df[greg_df.new_docno == row.previous_docno_str].current_document.values[0] #GREG DEBUG
                            prev_text = greg_df[greg_df.docno == row.previous_docno_str].current_document.values[0]
                    except:
            ...
            with torch.no_grad():
                ...
                if row['NEF@10_dense'] == 'nan' or row['NEF@10_max_dense'] == 'nan' or row['EF@10_dense'] == 'nan' or \
                    ...
                    metrics_dense = process_top_docs(top_docs_dense_docnos, greg_df, sentences, k, cache, online=online)
                    metrics_sparse = process_top_docs(top_docs_sparse_docnos, greg_df, sentences, k, cache, online=online)
